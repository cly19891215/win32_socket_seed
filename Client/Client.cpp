...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	//	W_USEDEFAULT, 0, CW_USEDEFAULT, 0, NULL, NULL, hInstance, NULL);
	hWnd = CreateWindow(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
	...
	case WM_CREATE:
		{
			...
			CreateWindowEx(WS_EX_CLIENTEDGE,
							"STATIC",
							"History, Context Receive From Server",
			...
			nResult = WSAAsyncSelect(Socket, hWnd, WM_SOCKET, (FD_CLOSE | FD_READ | FD_ACCEPT| FD_CONNECT));
			if (nResult)
			{
				MessageBox(hWnd,
	...
	case WM_SOCKET:
		{
			...
			{
				MessageBox(hWnd,
							  "Connection to server failed, please start server first!",
							  "Error",
							  MB_OK | MB_ICONERROR);
				SendMessage(hWnd, WM_DESTROY, NULL, NULL);
				break;
